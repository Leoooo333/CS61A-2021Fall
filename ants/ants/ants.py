    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert place.ant is None \
                   or (logic_xor(place.ant.can_contain(self), self.can_contain(place.ant))
                       and logic_xor(place.ant.is_container, self.is_container)), \
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        frontier = self.place
        index = 0
        while not frontier.is_hive:
            if len(frontier.bees) > 0:
                if self.min_range <= index <= self.max_range:
                    break
                else:
                    pass
            frontier = frontier.entrance
            index += 1
        if not frontier.is_hive:
            return random_bee(frontier.bees)  # REPLACE THIS LINE
        else:
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        hurt = amount if self.health > amount else amount + self.damage
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if self.ant_contained is None and self.is_container:
            return True
        else:
            return False
        # END Problem 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.isthere_queen:
            return None
        else:
            gamestate.isthere_queen = True
    ...
    def action(self, gamestate):
        ...
        tail = self.place.exit
        while tail is not None:
            if tail.ant is not None:
                target = tail.ant
                if not target.have_doubled:
                    target.buff()
                if target.is_container and target.ant_contained is not None \
                        and not target.ant_contained.have_doubled:
                    target.ant_contained.buff()
            tail = tail.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if not self.is_slowed:
                ...
                if gamestate.time % 2 == 0 and self.slow_turns > 0:
                    if not destination.is_hive:
                        self.move_to(destination)
                        if self.scare_turns > 0:
                            self.scare_turns -= 1
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC
        self.scare_turns = 0 if self.have_scared else length
    ...
    def insects_in_front(self):
        ...
        while current_place is not None:
            if current_place.ant is not None and current_place.ant is not self:
                insects.append(current_place.ant)
            ...
            index += 1
        return dict(zip(insects, distances))
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = 2
        damage = damage - 0.0625 * self.insects_shot if damage > 0.0625 * self.insects_shot else 0
        damage = damage - 0.25 * distance if damage > 0.25 * distance else 0
