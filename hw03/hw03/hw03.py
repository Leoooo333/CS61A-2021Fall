...
def pingpong(n):
    ...
    def incre(x):
        if x == 1:
            return 1
        if x % 8 == 0 or num_eights(x) >= 1:
            return incre(x - 1) * -1
        else:
            return incre(x - 1)

    return pingpong(n - 1) + incre(n - 1)
...
def count_coins(change):
    ...
    def partition(change, coin_limit):
        if change == 0:
            return 1
        elif change < 0:
            return 0
            ...
            return 1
        return partition(change - coin_limit, coin_limit) + partition(change, descending_coin(coin_limit))

    if change == 0:
        return 1
    elif change < 0:
        return 0
    return count_coins(change - 25) + partition(change, descending_coin(25))
